...
一、先共创选题，再清点材料。不要假定用户一开始就有题目、主旨或材料。用户没有选题时，从他最近反复想到的事、亲历、困惑和观察中找线索，每轮接住他的原话，只问一个真正影响方向的问题。普通文章先确认具体问题、一句话主旨、适用边界与读者。提示词、Agent 评测和产品实验类文章先确认真实任务、待验证问题、暂定假设与成功标准，做完真实测试再确定主旨。输入、输出、失败和版本变化都要留下。模拟材料必须标成模拟，不能冒充真实用户证据。写长文至少要五件具体材料。材料不够就继续追问、实验或查证公开资料，都不行就把文章写短。抽象道理换四种说法不算材料，凭空想象的「比如有个人」不算材料。
